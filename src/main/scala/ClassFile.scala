...
object ClassFile {
  ...
  object RawConstantPool {
    ...
    extension (rawPool: RawConstantPool) {
      ...
      def resolveUtf8(index: UShort): Result[PoolEntry.CONSTANT_Utf8_info] =
        rawPool.lift(index).flatten match {
      ...
      def resolveRawEntry(raw: PoolEntryRaw): Result[PoolEntry] = raw match {
        ...
        case PoolEntryRaw.CONSTANT_MethodHandle_info(referenceKind, referenceIndex) =>
          rawPool.lift(referenceIndex).flatten match {
  ...
  opaque type RawConstantPool = Vector[Option[PoolEntryRaw]]
  ...
  case class AttributeHeader(attributeName: String, len: UInt)
  ...
  object Attribute {
        ...
        read2Bytes(bytes, offset + 2, "code attr max_locals").andThen { maxLocals =>
          readUInt(bytes, offset + 4, "code attr code_length").andThen { codeLen =>
            ...
            read2Bytes(bytes, codeEnd, "code attr exception_table_length").andThen { exceptionTableLen =>
              ...
              (0 until exceptionTableLen).toList
                      ...
                      read2Bytes(bytes, entryOffset + 2, "exception_table end_pc").andThen { endPc =>
                        read2Bytes(bytes, entryOffset + 4, "exception_table handler_pc").andThen { handlerPc =>
                          read2Bytes(bytes, entryOffset + 6, "exception_table catch_type").andThen { catchTypeIdx =>
                            val catchType =
                              if catchTypeIdx == 0 then None.validNel
    ...
    def readExceptionsBody(bytes: Array[Byte], offset: Int, pool: ConstantPool, len: Int): Result[Attribute.Exceptions] =
      read2Bytes(bytes, offset, "exceptions attr number_of_exceptions").andThen { numberOfExceptions =>
        (0 until numberOfExceptions).toList
  ...
  opaque type ConstantPool = Vector[Option[PoolEntry]]

  def magicNumberMatch(bytes: Array[Byte]): Boolean = {
    bytes.take(4).map(_ & 0xff) sameElements Array(0xca, 0xfe, 0xba, 0xbe)
  }

  def read2Bytes(bytes: Array[Byte], offset: Int, label: String = "smthing"): Result[UShort] = {
  ...
  def readUInt(bytes: Array[Byte], offset: Int, label: String = "smthing"): Result[UInt] = {
  ...
  def readInt(bytes: Array[Byte], offset: Int, label: String = "smthing"): Result[Int] = {
    bytes.toList.slice(offset, offset + 4) match {
  ...
  def readFloat(bytes: Array[Byte], offset: Int, label: String = "smthing"): Result[Float] = {
  ...
  def readLong(bytes: Array[Byte], offset: Int, label: String = "smthing"): Result[Long] = {
  ...
  def readDouble(bytes: Array[Byte], offset: Int, label: String = "smthing"): Result[Double] = {
  ...
  def majorVersion(bytes: Array[Byte]): Result[UShort] = read2Bytes(bytes, 6, "major")

  def minorVersion(bytes: Array[Byte]): Result[UShort] = read2Bytes(bytes, 4, "minor")

  def getPoolsize(bytes: Array[Byte]): Result[UShort] = read2Bytes(bytes, 8, "pool size")

  type NextOffset = Int

  def readPoolEntry(bytes: Array[Byte], offset: Int): Result[(PoolEntryRaw, NextOffset)] =
  ...
  def readAllRawPoolEntry(bytes: Array[Byte],
                          poolSize: Int,
                          offset: Int = 10,
                          iter: Int = 0,
  ...
  def readAllPoolEntry(bytes: Array[Byte],
  ...
  enum ClassAccessFlag(val mask: Int):
  ...
  enum FieldAccessFlag(val mask: Int):
  ...
  case class ClassFileProperties(flags: List[ClassAccessFlag], thisClass: PoolEntry.CONSTANT_Class_info, superClass: Option[PoolEntry.CONSTANT_Class_info], interfaces: List[PoolEntry.CONSTANT_Class_info])

  def readInterfaces(bytes: Array[Byte], offset: Int, pool: ConstantPool): Result[(List[PoolEntry.CONSTANT_Class_info], NextOffset)] =
  ...
  def readProperties(bytes: Array[Byte], offset: Int, pool: ConstantPool): Result[(ClassFileProperties, NextOffset)] = {
  ...
  def readAttributeHeader(byte: Array[Byte], offset: Int, pool: ConstantPool): Result[(AttributeHeader, NextOffset)] = {
  ...
  case class FieldInfo(accessFlags: List[FieldAccessFlag],
                       name: PoolEntry.CONSTANT_Utf8_info,
                       descriptor: PoolEntry.CONSTANT_Utf8_info,
                       attributes: List[Attribute])

  def readAttribute(bytes: Array[Byte], offset: Int, pool: ConstantPool): Result[(Option[Attribute], NextOffset)] = {
  ...
  def readAttributes(bytes: Array[Byte], offset: Int, pool: ConstantPool): Result[(List[Attribute], NextOffset)] = {
  ...
  def readAndResolveUtf8Constant(bytes: Array[Byte], offset: Int, pool: ConstantPool): Result[PoolEntry.CONSTANT_Utf8_info] = {
    read2Bytes(bytes, offset, s"reading resolvable utf8 constant on offset  $offset").andThen { nameIdx =>
      pool.resolveUtf8(nameIdx)
    }
  }

  def readField(bytes: Array[Byte], offset: Int, pool: ConstantPool): Result[(FieldInfo, NextOffset)] = {
        ...
        readAndResolveUtf8Constant(bytes, offset + 2, pool).andThen { fieldName =>
          readAndResolveUtf8Constant(bytes, offset + 4, pool).andThen { descriptor =>
            readAttributes(bytes, offset + 6, pool).map { (attributes, nextOffset) =>
              (FieldInfo(accessFlags.toList, fieldName, descriptor, attributes), nextOffset)
            }
          }
  ...
  def readFields(bytes: Array[Byte], offset: Int, pool: ConstantPool): Result[(List[FieldInfo], NextOffset)] = {
  ...
  enum MethodAccessFlag(val mask: Int):
    ...
    read2Bytes(bytes, offset, s"reading method flags on offset  $offset")
      .andThen { methodAccessFlagRaw =>
        ...
        readAndResolveUtf8Constant(bytes, offset + 2, pool).andThen { methodName =>
          readAndResolveUtf8Constant(bytes, offset + 4, pool).andThen { descriptor =>
            readAttributes(bytes, offset + 6, pool).map { (attributes, nextOffset) =>
              (MethodInfo(accessFlags.toList, methodName, descriptor, attributes), nextOffset)
            }
          }
        }
      }
  ...
  def readMethods(bytes: Array[Byte], offset: Int, pool: ConstantPool): Result[(List[MethodInfo], NextOffset)] = {
